...
_archive_close_intents: WorkspaceScopedSet[str] = WorkspaceScopedSet()
...
async def _persist_external_conversation_item_locked(
    ...
    if "recovery_after" in body.data:
        ...
        except NativeRecoveryItemSkipped:
            return None, True
...
# Placeholder the parent inbox shows when a terminal edge carries no assistant
# text this turn can claim. Without one the parent renders an empty result and
# the operator cannot tell a silent success from a silent failure.
...
async def _enrich_terminal_status_with_subagent_output(
    ...
    fallback = TERMINAL_STATUS_FALLBACK_OUTPUTS.get(status)
