...
def _latest_assistant_text_from_store(
    ...
    *,
    ...
    stop_at_turn_boundary: bool = False,
) -> str | None:
    """
    ...
    :param stop_at_turn_boundary: Fork mod, widening upstream's
        ``stop_at_user_message``: without a response id, stop at the newest
        non-meta item that is not an assistant message — a user turn, or a
        tool call/result. Either one proves an older assistant message was an
        opener or an intermediate update, not this turn's terminal result.
        Off for the legacy ``idle`` edge, which reads past both as it always
        has. Tool items are why this scan does not filter to ``"message"``.
    ...
    strict = stop_at_turn_boundary and response_id is None
    for item in page.data:
        if not isinstance(item.data, MessageData):
            if strict:
                return None
