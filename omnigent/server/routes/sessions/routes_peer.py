    ...
    *,
        ...
        if not relaunchable:
            ...
            if native:
                ...
                if runner_client is None:
                    ...
                    try:
                        # No runner_router: a tunnel-drop wait (30 s) would outlast the runner's
                        # 30 s send budget; not_ready defers to the sweeper, and delivery itself
                        # retries via _dispatch_session_event_to_runner_impl.
    ...
    async def _send_peer_message_locked(
        ...
        async def _queued_response(
            ref: str, now: int, runner_online: bool | None
        ) -> dict[str, Any]:
