    ...
    *,
    ...
    async def create_session(
        ...
        if _rc is not None and conv is not None:
            ...
            init_body = _session_init_notify_body(
    ...
    async def switch_session_agent(
        ...
        # The plan belongs to the harness that wrote it, and persisted todos
        # outlive the process — so a switch clears them. That happens in
        # `_reset_runner_and_clear_todos_after_switch` below, AFTER the old
        # runner is torn down, because a clear here would be overwritten by a
        # late Plan POST from the runner still shutting down.
