...
def create_runner_app(
    *,
    ...
    async def get_session(session_id: str) -> JSONResponse:
        ...
        # Serve the same observed status that the live stream publishes; a
        # failed setup has no active turn, so its ``failed`` pane status
        # stays visible to server status probes.
    ...
    async def post_session_events(
        ...
        if body_type == "interrupt":
            # Fork mod (MOD-s19): read before the cancel pops it; the Claude idle guard
            # must not treat a pane parked on a question as an idle composer.
            _prompt_pending = conversation_id in _claude_prompt_waiters
            ...
            _interrupt_resp = await _native_interrupt_runner.interrupt(
                _harness, conversation_id, prompt_pending=_prompt_pending
            )
