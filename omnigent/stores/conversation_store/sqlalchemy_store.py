...
class SqlAlchemyConversationStore(ConversationStore):
    ...
    def create_conversation(
        ...
        # A child with no routing of its own inherits the parent's. The
        # ancestor walk below runs inside the write transaction; this read
        # only needs Omnigent meta, so it stays outside it.
        ...
        try:
            ...
            def insert_conversation(ap_sess: Session) -> SqlConversation:
                ...
                if parent_conversation_id is not None:
                    # Same intent as the standalone parent lookup this replaced,
                    # so it keeps that name: resolve the parent (and the rest of
                    # the chain) before the child row lands.
    ...
    def append(
        ...
        for item, data in zip(items, encoded_data, strict=True):
            ...
            # Our derivation, moved into upstream's precompute: an idempotency
            # key must map to the SAME row id on a re-post, and precomputing it
            # here is what makes a CockroachDB 40001 replay reuse it.
    ...
    def list_conversations(
        ...
        # still require Omnigent-side data are the permission scopes, plus the
        # host filter. shared_only (accessible − owned) is a permission scope too.
        needs_meta_filter = (
