...
class HostStore:
    ...
    def upsert_on_connect(
        ...
        *,
        ...
        def write(session: Session) -> Host:
            ...
            # Genuinely new host: plain INSERT. `cli_retention_revision` is
            # spelled out because `_row_to_host` reads the row before the
            # flush that would apply the column default, and the caller
            # compares the returned Host against a later re-read.
            row = SqlHost(
    ...
    def register_managed_host(
        ...
        *,
        ...
        def write(session: Session) -> Host:
            generation = require_active_account(session, user_id)
            # `cli_retention_revision` is spelled out for the same reason as
            # in `upsert_host_on_connect`: the Host is built from the row
            # before the flush that would apply the column default.
            row = SqlHost(
                account_generation=generation,
                user_id=user_id,
                name=name,
