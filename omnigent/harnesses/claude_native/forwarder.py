...
def _note_forward_failure(retry_key: str, exc: Exception, *, session_id: str) -> None:
    """
    Record a forward post failure; escalate once when sync degrades.

    :param retry_key: Stable retry key of the failed post, e.g.
        ``"item:source-1"``.
    :param exc: The latest failed post's exception.
...
class SubagentEntry:
    ...
    :param last_status: Last status handled for this sub-agent (``running``,
        ``idle``, ``activity_unverified`` or a terminal status), used to
        dedupe per tick. ``None`` means none handled yet.
...
class SubagentForwardState:
    ...
    pending_terminal_parent_offsets: dict[str, int] = field(default_factory=dict)
...
class _SubagentStateCheckpoint:
    ...
    async def put(self, entry: SubagentEntry) -> None:
        ...
        async with self._lock:
            self._state = replace(
                self._state, subagents={**self._state.subagents, entry.subagent_id: entry}
            )
...
async def forward_claude_transcript_to_session(
    *,
    ...
    snapshot_parent_ids: set[str] = set()
            ...
            try:
                if subagent_task is not None and subagent_task.done():
                    try:
                        ...
                        grouped_snapshots = _native_subagent_snapshots_by_parent(
                            session_id,
                            subagent_state,
                        )
                        for parent_id, children in grouped_snapshots.items():
                            snapshots.update(parent_id, children)
                        snapshot_parent_ids.update(grouped_snapshots)
                    subagent_task = None
                async with _forward_progress_timeout(client, _FORWARD_LOOP_STALL_DEADLINE_S):
                    ...
                    if rotation is not None:
                        ...
                        _retire_native_subagent_snapshots(
                            snapshots,
                            root_parent_id=session_id,
                            parent_ids=snapshot_parent_ids,
                        )
                        session_id = rotation
                        state = None
                        hook_state = None
...
def _native_subagent_snapshot(state: SubagentForwardState) -> dict[str, str]:
    ...
    return {
        entry.child_conversation_id: _native_subagent_status(entry)
        for entry in state.subagents.values()
        if entry.child_conversation_id
    }


def _native_subagent_status(entry: SubagentEntry) -> str:
    """Return observation status without inventing a terminal outcome."""
    return entry.terminal_status or (
        "activity_unverified"
...
def _native_subagent_snapshots_by_parent(
    root_parent_id: str,
    state: SubagentForwardState,
) -> dict[str, dict[str, str]]:
    """Group each child under its immediate Omnigent parent in linear time."""
    grouped: dict[str, dict[str, str]] = {}
    for entry in state.subagents.values():
        if not entry.child_conversation_id:
            continue
        if entry.parent_subagent_id is None:
            parent_id = root_parent_id
        else:
            parent_entry = state.subagents.get(entry.parent_subagent_id)
            parent_id = parent_entry.child_conversation_id if parent_entry is not None else ""
        if not parent_id:
            continue
        grouped.setdefault(parent_id, {})[entry.child_conversation_id] = _native_subagent_status(
            entry
        )
    return grouped


def _retire_native_subagent_snapshots(
    snapshots: NativeSubagentSnapshotPublisher,
    *,
    root_parent_id: str,
    parent_ids: set[str],
) -> None:
    """Retire every parent inventory owned by the superseded native session."""
    for parent_id in sorted(parent_ids | {root_parent_id}):
        snapshots.update(parent_id, {}, retired=True)
    parent_ids.clear()
...
def _read_subagent_forward_state(bridge_dir: Path) -> SubagentForwardState:
    ...
    offsets = raw.get("pending_terminal_parent_offsets", {})
    offsets = (
        {k: v for k, v in offsets.items() if isinstance(k, str) and isinstance(v, int) and v >= 0}
    ...
    return SubagentForwardState(
        ...
        pending_terminal_parent_offsets=offsets,
...
def _write_subagent_forward_state(bridge_dir: Path, state: SubagentForwardState) -> None:
    ...
    payload: dict[str, object] = {
        ...
        "pending_terminal_parent_offsets": state.pending_terminal_parent_offsets,
...
async def _write_subagent_forward_state_async(
    ...
    writer = asyncio.create_task(
        asyncio.to_thread(_write_subagent_forward_state, bridge_dir, state),
        name="claude-subagent-state-write",
    )
    try:
        await asyncio.shield(writer)
    except asyncio.CancelledError:
        # ``to_thread`` cannot stop a write already running in its worker.
        # Keep cancellation from releasing the checkpoint lock until that write
        # finishes, otherwise a rotation's empty reset can be overwritten later.
        while not writer.done():
            try:
                await asyncio.shield(writer)
            except asyncio.CancelledError:
                # A supervisor may repeat cancellation while cleanup is draining.
                continue
        try:
            writer.result()
        except Exception:
            _logger.exception("Claude sub-agent state write failed during cancellation")
        raise
...
async def _post_external_conversation_item_batch(
    ...
    *,
    ...
    return [row.get("replayed") is not True for row in acknowledgements]
...
async def _post_external_conversation_items(
    ...
    *,
    ...
    """Post a batch, or request individually checkpointed delivery for old Servers."""

    if batch_capability.supported is False:
        return None
    try:
        accepted = await _post_external_conversation_item_batch(
            client,
            session_id=session_id,
            items=items,
        )
...
async def _forward_one_subagent(
    *,
    ...
    if not jsonl_path.exists():
        ...
        return
    recovered = await _recover_subagent_history(
        client=client,
        entry=entry,
    ...
    for batch in batches:
        ...
        retry_individually = batch_capability.supported is False or any(
        ...
        if drop_reason is not None:
            ...
            try:
                accepted = await _post_external_conversation_items(
                    client,
                    session_id=entry.child_conversation_id,
                    items=batch,
                ...
                if accepted is None:
                    ...
                    completed_items.extend(batch)
                    delivered_now.extend(
                        item for item, is_new in zip(batch, accepted, strict=True) if is_new
                    )
        ...
        new_entry = replace(
            ...
            quiet_terminal_output=(
                _subagent_quiet_terminal_output(completed_items[-1].item)
                if completed_items
            ...
            status_reconcile_pending=(
                new_entry.status_reconcile_pending or (delivered and new_entry.activity_unverified)
            ),
        )
        await checkpoint.put(new_entry)
        if stop_after_batch:
    ...
    *,
    ...
    if body.get("skipped") is True:
        if (
            ...
            or "item_id" not in body
            or item_id is not None
            or body.get("queued") is not False
        ):
            raise RuntimeError("sub-agent history recovery response did not confirm the chain")
        return None
    if "skipped" in body or not isinstance(item_id, str) or not item_id:
        raise RuntimeError("sub-agent history recovery response did not confirm the chain")
    return item_id
    ...
    *,
    ...
    for sid, entry in entries.items():
        ...
        if not entry.child_conversation_id or (
            entry.terminal_status is not None and entry.last_status in _SUBAGENT_TERMINAL_STATUSES
        ):
    ...
    state = replace(
        ...
        subagents={
            sid: replace(
                entry,
                parent_recovery_watermark=max(
                    state.parent_byte_offset, entry.parent_recovery_watermark or 0
                )
                or None,
            )
            if initial
...
async def _recover_subagent_history(
    *,
    client: httpx.AsyncClient,
    entry: SubagentEntry,
    jsonl_path: Path,
    ...
    """Reconcile the frozen history before the live batch lane can advance."""
    if entry.recovery_watermark is not None:
        ...
        unmatched = [item for item in recovery_result.items if item.source_id not in recovery_seen]
        ...
        for item in unmatched[:_SUBAGENT_RECOVERY_BATCH_ITEMS]:
            ...
            entry = replace(
                entry,
                recovery_after=(
                    recovery_item_id if recovery_item_id is not None else entry.recovery_after
                ),
...
async def _publish_subagent_status(
    *,
    ...
    if entry.terminal_status in _SUBAGENT_TERMINAL_STATUSES:
        desired_status = entry.terminal_status
        desired_output = entry.terminal_output
        desired_replayed = entry.terminal_replayed
    elif entry.activity_unverified:
    ...
    # transport backoff rather than the child going quiet.
    elif (
        not delivery_pending
        and entry.last_activity_ts is not None
        and time.time() - entry.last_activity_ts > _SUBAGENT_IDLE_THRESHOLD_S
    ):
        ...
        desired_status = "failed" if entry.delivery_error else "idle"
        ...
        if status_retry_tracker.retry_delay_s(retry_key) is None:
            try:
                if desired_status == "idle":
                    await status_capability.post_idle(
                        client, session_id=entry.child_conversation_id
                    )
                else:
    ...
    if entry is not original_entry:
        await checkpoint.put(entry)
...
async def _forward_available_subagents(
    *,
    ...
    pending_aliases = dict(updated.pending_terminal_tool_use_ids)
    pending_offsets = dict(updated.pending_terminal_parent_offsets)
    for notification in () if parent_result is None else parent_result.task_notifications:
        ...
        old_parked = pending_notifications.get(park_key or "")
        if (
            park_key is not None
            and old_parked is not None
            and len(old_parked) == 4
            and (old_parked[0], old_parked[1], old_parked[3])
            == (status, notification.result, notification.timestamp)
        ):
            if notification.tool_use_id is not None:
                pending_aliases.setdefault(park_key, notification.tool_use_id)
            if parent_result is not None:
                pending_offsets.setdefault(park_key, parent_result.byte_offset)
        ...
        if (
            ...
            and not target_entry.terminal_evidence_pending
        ):
            if not _terminal_is_current(target_entry, outcome[3]):
                continue
            entries[target_id] = replace(
                ...
                status_reconcile_pending=target_entry.status_reconcile_pending
                or (
                    target_entry.terminal_status,
                    target_entry.terminal_output,
                    target_entry.terminal_observed_at,
                )
                != (outcome[0], outcome[1], outcome[3]),
                activity_unverified=False,
            )
        else:
            park_key = notification.task_id or notification.tool_use_id
            if park_key is not None:
                old = pending_notifications.get(park_key)
                if (
                    old is not None
                    and len(old) == 4
                    and _parse_record_timestamp(old[3]) is not None
                    and not _record_timestamp_is_newer(outcome[3], old[3])
                ):
                    continue
                pending_notifications[park_key] = outcome
                if parent_result is not None:
                    pending_offsets[park_key] = parent_result.byte_offset
                if notification.tool_use_id is not None:
                    pending_aliases[park_key] = notification.tool_use_id
    for subagent_id, entry in entries.items():
        if entry.terminal_evidence_pending:
            continue
        keys = [
            key
            for key in pending_notifications
            if key == subagent_id
            or (
                entry.tool_use_id is not None
                and (key == entry.tool_use_id or pending_aliases.get(key) == entry.tool_use_id)
            )
        ]
        selected = (
            subagent_id
            if subagent_id in keys
            else max(
                keys,
                key=lambda key: (
                    _parse_record_timestamp(
                        pending_notifications[key][3]
                        if len(pending_notifications[key]) == 4
                        else None
                    )
        ...
        notification = pending_notifications[selected] if selected is not None else None
        observed_offset = pending_offsets.get(selected) if selected is not None else None
        for key in keys:
            pending_notifications.pop(key)
        ...
        terminal_replayed = terminal_replayed or (
            observed_offset is not None
            and entry.parent_recovery_watermark is not None
            and observed_offset <= entry.parent_recovery_watermark
        )
        if not _terminal_is_current(entry, observed_at):
            continue
    ...
    updated = replace(
        ...
        pending_terminal_tool_use_ids=pending_aliases,
        pending_terminal_parent_offsets=pending_offsets,
    )
        ...
        updated = replace(
            ...
            subagents={
                subagent_id: (
                    ...
                    and entry.tool_use_id is not None
                    and not entry.terminal_evidence_pending
                    else entry
                )
                for subagent_id, entry in updated.subagents.items()
            },
...
async def _forward_available_status_events(
    *,
    ...
    for record in result.records:
        ...
        # A record is a child's when its Claude session id was never pinned to
        # this bridge (background sub-agent) or its transcript sits under
        # ``subagents/`` (synchronous sub-agent).
...
async def _handle_compact_summary_item_unlocked(
    ...
    * Pending token present → attempt the boundary persist. On success,
    ...
    try:
        ...
        retry_tracker.record_failure(retry_key, exc, session_id=session_id)
...
def _http_status_for_log(exc: Exception) -> int | None:
    """
