...
Pi / cursor / kiro / goose reach the channel through their launch adapters.
"""
...
_HARNESSES = ["pi", "cursor", "kiro", "goose"]
_LAUNCHERS = {
    "pi": _launch_pi,
...
def _context(agent_spec: AgentSpec | None, global_instructions: str | None) -> Any:
...
async def test_launch_stages_startup_preamble(
    ...
    with pytest.raises(_StopAfterStaging):
        await _LAUNCHERS[harness](_context(_spec(), "GLOBAL-MARKER"))
...
async def test_launch_without_spec_or_global_text_stages_nothing(
    ...
    with pytest.raises(_StopAfterStaging):
        await _LAUNCHERS[harness](_context(None, None))
