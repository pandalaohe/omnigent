...
async def test_grant_gate_mirrors_peer_messaging_flag(
    monkeypatch: pytest.MonkeyPatch, enabled: bool
) -> None:
    """Peers: the grant gate admits ``sys_session_send`` only while the flag is on."""
    from omnigent.spec.types import AgentSpec

    reached: list[dict[str, Any]] = []

    async def _record(args: dict[str, Any], **_kwargs: Any) -> str:
        reached.append(args)
        return json.dumps({"ok": True})

    monkeypatch.setattr(tool_dispatch, "_execute_subagent_tool", _record)
    monkeypatch.setattr(tool_dispatch, "_peer_messaging_enabled_for", lambda _cid: enabled)

    out = await tool_dispatch.execute_tool(
        tool_name="sys_session_send",
        arguments=json.dumps({"session_id": _TARGET, "args": "hi"}),
...
# Grant gate (_ungranted_tool_reason on the sys_call_async path) follows the same flag.
@pytest.mark.asyncio
async def test_grant_gate_admits_async_peer_send_when_flag_on(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Peers: ``sys_call_async`` targeting ``sys_session_send`` is not refused with the flag on."""
    import asyncio as _asyncio

    from omnigent.spec.types import AgentSpec

    async def _record(_args: dict[str, Any], **_kwargs: Any) -> str:
        return json.dumps({"ok": True})

    monkeypatch.setattr(tool_dispatch, "_execute_subagent_tool", _record)
    monkeypatch.setattr(tool_dispatch, "_peer_messaging_enabled_for", lambda _cid: True)
    inbox: _asyncio.Queue[dict[str, Any]] = _asyncio.Queue()
    tasks: dict[str, tuple[_asyncio.Task[str], _asyncio.Event]] = {}

    out = await tool_dispatch.execute_tool(
        tool_name="sys_call_async",
        arguments=json.dumps({"tool": "sys_session_send", "args": "{}"}),
        agent_spec=AgentSpec(spec_version=1, async_enabled=True),
        conversation_id=_CALLER,
        session_inbox=inbox,
