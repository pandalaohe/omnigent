...
async def test_auto_create_claude_terminal_forwarder_skips_replayed_transcript_on_resume(
    ...
    async def _fake_synth(
        ...
        *,
        ...
        if reuse_local:
            ...
            bridge = claude_native_bridge.bridge_dir_for_conversation_id(
            ...
            _write_forward_state(
                ...
                TranscriptForwardState(
                    ...
                    line_cursor=0,
                    byte_offset=0,
                    cursor_fingerprint=_jsonl_cursor_fingerprint(transcript, 0),
                ),
            )
        return ClaudeResumeTranscriptResolution(
            transcript,
            reused_local=reuse_local,
            synthesized=not reuse_local,
        )
    ...
    if snapshot_external_id is not None and reuse_local:
        ...
        assert saved is not None and saved.byte_offset == 0
    ...
    *before* prepare_bridge_dir wipes the file and use it as the resume
    ...
    async def _fake_synth(
        ...
        *,
        ...
        transcript = tmp_path / f"{external_session_id}.jsonl"
        transcript.write_text('{"type":"user","message":{"content":"fixture"}}\n')
        return ClaudeResumeTranscriptResolution(transcript, reused_local=False, synthesized=True)
