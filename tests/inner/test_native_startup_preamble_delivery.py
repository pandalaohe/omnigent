...
Pi / cursor / kiro / goose / hermes / qwen prepend the staged text to the first
message they inject and clear it only after that injection lands, so a failed
first turn retries with the instructions instead of losing them.
"""
...
_CASES = [
    ...
    pytest.param(HermesNativeExecutor, hne, "inject_user_message", False, id="hermes"),
    pytest.param(QwenNativeExecutor, qne, "submit_user_message", False, id="qwen"),
]


def _prime_executor(executor_cls: type[Any], ex: Any) -> None:
    """Skip qwen's boot-detection wait — irrelevant to preamble wrapping."""
    if executor_cls is QwenNativeExecutor:
        ex._ready = True
...
async def test_first_turn_delivers_staged_preamble_once(
    ...
    _prime_executor(executor_cls, ex)
