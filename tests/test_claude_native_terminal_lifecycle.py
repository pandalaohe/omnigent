...
async def test_upgrade_preserves_a_replayed_prompt_acknowledgement(tmp_path):
    scene = _Scene(tmp_path)
    scene.register()
    await scene.tick(_terminal())
    _append(scene.child, _resume())
    await scene.tick(replayed_items=True)
    assert scene.state.subagents["worker"].terminal_status == "completed"
    scene.reload(legacy=True)
    scene.events.clear()
    await scene.tick()
...
async def test_foreground_without_agent_id_uses_the_spawn_tool(tmp_path):
    scene = _Scene(tmp_path)
    scene.register()
    await scene.tick(
        {
            ...
            "message": {
                "role": "user",
                "content": [
                    {"type": "tool_result", "tool_use_id": "tool_spawn", "content": "first done"}
                ],
            },
