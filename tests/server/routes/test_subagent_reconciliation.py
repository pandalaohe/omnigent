...
async def test_reconcile_route_reports_old_host_without_changing_state(
    ...
    assert "Update the Host" in response.text
    unchanged = store.get_conversation(child.id)
...
def test_reconcile_cas_rejects_new_running_edge_without_new_item(db_uri: str) -> None:
    ...
    result = store.reconcile_native_subagent_status(
        frozen,
        live_status="idle",
        label_updates={_TERMINAL_KEY: "completed"},
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("denied_child", [False, True])
async def test_reconcile_requires_owner_on_parent_and_each_child_before_host_probe(
    client: httpx.AsyncClient,
    db_uri: str,
    monkeypatch: pytest.MonkeyPatch,
    denied_child: bool,
    ...
    store = SqlAlchemyConversationStore(db_uri)
    parent = await _seed_parent_via_api(client, store, "owner-gate-parent")
    child = _seed_native_child(store, parent_id=parent["id"], agent_id=parent["agent_id"])
    store.set_session_live_status(child.id, "running")
    original_access = routes_items_module._require_access_and_level
    checked: list[str] = []

    async def gated_access(user_id: Any, session_id: str, level: int, *args: Any) -> Any:
        assert level == LEVEL_OWNER
        checked.append(session_id)
        if session_id == (child.id if denied_child else parent["id"]):
            raise OmnigentError("Owner access required", code=ErrorCode.FORBIDDEN)
        return await original_access(user_id, session_id, level, *args)

    async def forbidden_probe(*args: Any, **kwargs: Any) -> Any:
        raise AssertionError("unauthorized request contacted the Host")

    monkeypatch.setattr(routes_items_module, "_require_access_and_level", gated_access)
    monkeypatch.setattr(routes_items_module, "_get_runner_client", forbidden_probe)
    response = await client.post(f"/v1/sessions/{parent['id']}/child_sessions/reconcile")
    assert response.status_code == 403
    assert checked == ([parent["id"], child.id] if denied_child else [parent["id"]])
    assert store.get_conversation(child.id).live_status == "running"  # type: ignore[union-attr]


def test_reconcile_refuses_split_database_without_writes(db_uri: str) -> None:
    ...
    frozen = store.get_native_subagent_reconcile_fingerprint(child.id, _FINGERPRINT_LABEL_KEYS)
    assert frozen is not None
    split_engine = create_engine("sqlite://")
    original_engine = store._engine
    try:
        store._engine = split_engine
        assert (
            store.reconcile_native_subagent_status(
                frozen, live_status="idle", label_updates={_TERMINAL_KEY: "completed"}
            )
            == "unsupported"
        )
    finally:
        store._engine = original_engine
