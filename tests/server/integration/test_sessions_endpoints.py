...
async def test_in_flight_todos_update_cannot_repopulate_deleted_session_cache(
    ...
    """A todo write that loses a delete race must not resurrect the session."""
    agent = await create_test_agent(client)
    ...
    # The store refuses a write to a deleted row and the handler returns
    # early, so the losing write leaves nothing behind to read back.
...
async def test_patch_runner_rebind_clears_stale_failed_status(
    ...
    assert init_post["json"]["session_init"]["session_id"] == sid
    assert init_post["json"]["session_init"]["agent_id"] == agent["id"]
...
async def test_post_external_session_status_idle_forwards_persisted_assistant_output(
    ...
    assert child_snapshot.json()["labels"]["omnigent.subagent.terminal_status"] == "completed"
    assert forwarded == [
        {
