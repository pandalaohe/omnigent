...
- [260912] **Upstream-parity repair** (board OMN07 / task T260912-021, commits `5a69c9cc5` `59856e8d8` `45ff246a9` `0ed8c3fc2`). Two findings the v2 rebuild could not see, because both look like upstream code rather than a local mod. (1) A merge dropped one hunk of upstream `ff0bbabd2f` (#6614): the docstring, the forwarder change and the tests landed, but `CompactionData.window_id` stayed `int | None` while current Codex writes a UUID string, so every Codex-native compaction POST was answered 400 and the summary plus the replacement history were dropped with only a warning (observed live on 2026-09-03 in the host runner log). Restored to upstream's `int | str | None`; this is a divergence removed, so it claims no row. (2) The fork-local stop rescue in `S21-claude-native-control` had silently overridden upstream #6676's conditional settle; it is now gated — see that row. Lesson for the next rebuild: a lost upstream hunk is invisible to path-ownership derivation, because the path still reads as upstream's. Only a test run against upstream's own suite surfaces it.
