...
/** The rail tab a session with nothing remembered opens on. Two preferences
    meet here: "carry my last pick forward" (on by default) only applies once
    there IS a pick, so until then — and whenever it is off — the Appearance
    default decides. */
function inheritedOrDefaultRightRailTab(): RightRailTab {
  const lastExplicit = readInheritLastRightRailTab() ? readLastExplicitRightRailTabOrNull() : null;
  return lastExplicit ?? readDefaultWorkspaceTab();
}
...
export function AppShell() {
  ...
  const [rightRailTab, setRightRailTab] = useState<RightRailTab>(() => {
    ...
    return inheritedOrDefaultRightRailTab();
  });
  ...
  useEffect(() => {
    ...
    // A remembered per-session tab wins. Navigating within the visible Agents
    // tree is one continuous rail action, so keep that tab while moving
    // between its root and descendants; otherwise inherit the last explicit
    // pick if enabled, else fall back to the Appearance default.
