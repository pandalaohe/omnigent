...
describe("useArchiveConversation", () => {
  ...
  it("filters stale refetches without stopping pagination", async () => {
    ...
    fetchMock.mockResolvedValueOnce(mockResponse(staleListPage));
    const archiveLibrary = renderHook(
      () =>
        useArchivedConversations({
          dateField: "created_at",
          sortField: "created_at",
    ...
    await waitFor(() => expect(archiveLibrary.result.current.isSuccess).toBe(true));
    expect(archiveLibrary.result.current.data!.data.map((row) => row.id)).toEqual(["conv_a"]);
    expect(archiveLibrary.result.current.data!.data[0].archived).toBe(true);
    archiveLibrary.unmount();
